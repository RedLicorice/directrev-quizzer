...
import { Flag, ChevronLeft, ChevronRight, LayoutList, X, Send } from 'lucide-react';
...
export default function ExamSession({ questions, config, onFinish, onExit }: Props) {
  ...
  const toggleFlag = useCallback(() => {
    setFlagged((f) => { const s = new Set(f); s.has(currentIdx) ? s.delete(currentIdx) : s.add(currentIdx); return s; });
  }, [currentIdx]);

  function handleSubmit() {
    onFinish({ mode: 'exam', questions, answers, config, timeElapsed: Math.round((Date.now() - startTime) / 1000) });
  }

  const getStatus = useCallback((idx: number): QuestionStatus => {
    if (flagged.has(idx)) return 'flagged';
    if ((answers[idx] ?? []).length > 0) return 'answered';
    return 'unanswered';
  }, [answers, flagged]);

  const timerColor = remaining === null ? 'text-slate-300'
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="sticky top-0 z-10 glass border-b border-slate-700/50 px-4 py-3">
        <div className="max-w-3xl mx-auto flex items-center gap-3">
          ...
          {config.timeLimit > 0 && remaining !== null && (
            <div className={`font-mono font-semibold text-sm tabular-nums ${timerColor}`}>{formatSeconds(remaining)}</div>
          )}

          <span className="text-sm text-slate-400 font-medium tabular-nums">{currentIdx + 1}/{questions.length}</span>
      ...
      <div className="sticky bottom-0 glass border-t border-slate-700/50 px-4 py-3">
        <div className="max-w-3xl mx-auto flex items-center gap-3">
          <button onClick={() => setCurrentIdx((i) => i - 1)} disabled={currentIdx === 0} className="btn-secondary flex items-center gap-1.5 px-4">
            <ChevronLeft className="w-4 h-4" /> Prev
          </button>

          <button
            onClick={toggleFlag}
            className={`btn-ghost flex items-center gap-1.5 px-3 text-sm ${flagged.has(currentIdx) ? 'text-amber-400' : ''}`}
          >
          ...
          {currentIdx < questions.length - 1 ? (
            <button onClick={() => setCurrentIdx((i) => i + 1)} className="btn-secondary flex items-center gap-1.5 px-4">
              Next <ChevronRight className="w-4 h-4" />
            </button>
          ) : (
            <button onClick={() => setShowSubmitConfirm(true)} className="btn-primary flex items-center gap-2 px-5">
      ...
      {showSubmitConfirm && (
        <Modal title="Submit Exam?" onClose={() => setShowSubmitConfirm(false)}>
          {unanswered > 0 && <p className="text-amber-400 text-sm mb-4">⚠️ {unanswered} unanswered question{unanswered > 1 ? 's' : ''}.</p>}
